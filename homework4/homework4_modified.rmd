...
pivot wider, calculate the difference between 2019 and 2010 and convert iso-3 to iso-2
```{r}
gender_index_wide <- gender_inequality_index %>%
  ...
  mutate(iso_2 = countrycode(country, origin = 'country.name', destination = 'iso2c'))
```
...
countries_join <- world_countries %>%
  left_join(.,
            gender_index_wide,
            by = c("ISO"="iso_2"))  #can't join with country names, using iso code
